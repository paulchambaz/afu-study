...
class ApproximatorNetwork(Agent):
    """Neural network that outputs a Gaussian distribution over actions."""
...
class AFU_Beta:
    def __init__(self, hyperparameters: OmegaConf):
        ...
        self.approximator_network = ApproximatorNetwork(
            state_dim=self.state_dim,
            hidden_dims=hidden_dims,
            action_dim=self.action_dim,
        )
        ...
        self.approximator_optimizer = torch.optim.Adam(
            self.approximator_network.parameters(), lr=self.params.approximator_lr
        )
    ...
    def update(self):
        ...
        # Update approximator network
        approximator_loss = self._compute_approximator_loss(states, actions)
        self.approximator_optimizer.zero_grad()
    ...
    def _compute_policy_loss(self, states: torch.Tensor) -> torch.Tensor:
        """
        Compute the policy loss with modified gradient to avoid deceptive gradients.
        ...
        # Compute the approximator predictions μζ(s)
        approximator_workspace = Workspace()
        approximator_workspace.set("env/env_obs", 0, states)
        self.approximator_network(approximator_workspace, t=0)
        self.approximator_network.sample_action(approximator_workspace, t=0)
        mu_zeta = approximator_workspace.get("action", 0)

        # Compute the minimum V-values for the states
        v_workspace1 = Workspace()
        v_workspace1.set("env/env_obs", 0, states)
        self.v_network1(v_workspace1, t=0)
        v_values1 = v_workspace1.get("v1/v_value", 0)

        v_workspace2 = Workspace()
        v_workspace2.set("env/env_obs", 0, states)
        self.v_network2(v_workspace2, t=0)
        v_values2 = v_workspace2.get("v2/v_value", 0)

        min_v_values = torch.min(v_values1, v_values2)

        # Compute the gradient adjustment operator Gs,as
        grad_q = torch.autograd.grad(
            q_values.sum(), actions, create_graph=True, retain_graph=True
        )[0]
        ...
        # Replace the gradient of the policy loss with the modified gradient
        policy_loss.backward(retain_graph=True)
        for param in self.policy_network.parameters():
            if param.grad is not None:
                param.grad = torch.autograd.grad(
                    policy_loss, param, grad_outputs=grad_q_adjusted, retain_graph=True
                )[0]
    ...
    def _compute_approximator_loss(self, states: torch.Tensor, actions) -> torch.Tensor:
        ...
        μ(ζ) = Mean(s,a•)∈M(B) [(μζ(s) - a•)2]
        """
        # Create a workspace to compute approximator distribution : log approximator_theta (a_s | s)
        approximator_workspace = Workspace()
        approximator_workspace.set("env/env_obs", 0, states)
        self.approximator_network(approximator_workspace, t=0)

        # Sample actions from the approximator
        self.approximator_network.sample_action(approximator_workspace, t=0)
        approximator_actions = approximator_workspace.get("action", 0)

        # Create a workspace to compute Q-values for actions in the mini-batch
        q_workspace_batch = Workspace()
        q_workspace_batch.set("env/env_obs", 0, states)
        q_workspace_batch.set("action", 0, actions)
        self.q_network(q_workspace_batch, t=0)
        q_values_batch = q_workspace_batch.get("q_value", 0)

        # Create a workspace to compute Q-values for approximator actions
        q_workspace_approximator = Workspace()
        q_workspace_approximator.set("env/env_obs", 0, states)
        q_workspace_approximator.set("action", 0, approximator_actions)
        self.q_network(q_workspace_approximator, t=0)
        q_values_approximator = q_workspace_approximator.get("q_value", 0)
        ...
        # Filter state-action pairs where Q(s, a) > min V(s)
        mask_batch = q_values_batch > min_v_values
        mask_approximator = q_values_approximator > min_v_values

        # Combine actions from the batch and approximator actions
        filtered_actions = torch.cat(
            [actions[mask_batch], approximator_actions[mask_approximator]], dim=0
        )
        filtered_states = torch.cat(
            [states[mask_batch], states[mask_approximator]], dim=0
        )
    ...
    def get_weights(self) -> dict:
        return {
            ...
            "approximator_network_state": self.approximator_network.state_dict(),
            ...
            "approximator_optimizer_state": self.approximator_optimizer.state_dict(),
