...
class AntEnvStudy(AntEnv):
    ...
    def set_state(
        ...
        if x_pos is not None and y_pos is not None:
            qpos = np.array([
                x_pos, y_pos, z_pos,                    # Cartesian position
                orientation_w, orientation_x, orientation_y, orientation_z,  # Quaternion orientation
                front_left_hip, front_left_ankle,       # Front left leg
                front_right_hip, front_right_ankle,     # Front right leg
                back_left_hip, back_left_ankle,         # Back left leg
                back_right_hip, back_right_ankle        # Back right leg
            ])
        else:
            # Keep current x,y position
            qpos = np.array([
                self.data.qpos[0], self.data.qpos[1], z_pos,  # Keep current x,y
                orientation_w, orientation_x, orientation_y, orientation_z,
                front_left_hip, front_left_ankle,
        ...
        qvel = np.array([
            x_vel, y_vel, z_vel,                    # Linear velocity
            angular_vel_x, angular_vel_y, angular_vel_z,  # Angular velocity
            front_left_hip_vel, front_left_ankle_vel,    # Front left leg
            front_right_hip_vel, front_right_ankle_vel,  # Front right leg
            back_left_hip_vel, back_left_ankle_vel,      # Back left leg
            back_right_hip_vel, back_right_ankle_vel     # Back right leg
        ])
