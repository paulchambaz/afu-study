...
class LunarLanderContinuousStudy(LunarLander):
    ...
    def set_state(
        ...
        world_y = (y_pos * viewport_h/2) + (self.helipad_y + self.LEG_DOWN/self.SCALE)
    ...
    def get_observation_space(self):
        ...
        return (
            np.array([-2.5, -2.5, -10.0, -10.0, -2*np.pi, -10.0, 0.0, 0.0], dtype=np.float32),
            np.array([2.5, 2.5, 10.0, 10.0, 2*np.pi, 10.0, 1.0, 1.0], dtype=np.float32)
        )
