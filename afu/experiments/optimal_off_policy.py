...
class OptimalOffPolicy(Experiment):
    def run(self, i, shared_results, results_lock, manager):
        ...
        if "afu" in algo_name_short:
            algo_name_short = "afu"
        if "sac" in algo_name_short:
            algo_name_short = "sac"

        env_name_short = self.params.env_name.split("-")[0].lower()

        if "pendulum" in env_name_short:
            env_name_short = "pendulum"
        if "lunarlander" in env_name_short:
            env_name_short = "lunarlander"

        optimal_model_path = f"afu/agents/{algo_name_short}-{env_name_short}-weights.pt"

        optimal_model = torch.load(optimal_model_path)

        obs_scale_factor = 1.0

        training_steps = 0
        agent = self.algo(self.hyperparameters, optimal_model)
