...
=== Impact attendu
La validation de ces hypothèses aurait un impact positif dans le domaine de l'apprentissage par renforcement :
...
- une transition plus fluide entre les phases d'apprentissage _offline_ et _online_, réduisant ainsi les coûts et les risques associés à cette transition.
...
_BBRL_ (_BlackBoard Reinforcement Learning_) est une bibliothèque conçue à des fins éducatives pour faciliter l'implémentation des algorithmes d'apprentissage par renforcement. Elle adopte une approche _Blackboard_ où différents agents interagissent dans un espace de travail partagé. Cette architecture modulaire facilite la conception et l'implémentation d'algorithmes complexes tout en maintenant une structure claire et compréhensible.

Nous utiliserons Python avec PyTorch comme framework de _deep learning_. PyTorch est une bibliothèque _open-source_ de _machine learning_ largement utilisée en recherche. Elle offre une grande flexibilité et une facilité d'utilisation pour le développement d'algorithmes d'apprentissage profond, avec des fonctionnalités avancées comme la différentiation automatique et le support GPU.
...
La seconde expérience explorera la stabilité d'AFU lors de la transition entre apprentissage _offline_ et _online_. L'apprentissage _offline_ est devenu un domaine de recherche important ces dernières années, car il permet d'entraîner des agents sans interaction coûteuse ou risquée avec l'environnement réel. Cependant, les algorithmes actuels comme _CQL_ ou _IQL_ souffrent souvent d'une chute de performance lors du passage à l'apprentissage _online_.

Cette dégradation s'explique notamment par le fait que ces algorithmes, pour éviter la surestimation des valeurs des actions non présentes dans la base de données, ont tendance à sous-estimer systématiquement ces valeurs. Lors du passage à l'apprentissage _online_, ces valeurs sont soudainement mises à jour, créant un déséquilibre qui perturbe l'apprentissage. AFU, grâce à son mécanisme d'estimation des valeurs maximales, pourrait offrir une estimation plus précise même pour les actions non vues, facilitant ainsi la transition vers l'apprentissage _online_.

Pour tester cette hypothèse, nous entraînerons d'abord AFU sur un ensemble de données pré-collectées, puis nous passerons à un apprentissage _online_ en mesurant précisément l'évolution des performances pendant cette transition. Si AFU maintient des performances stables, cela validera sa robustesse face à ce défi de l'apprentissage par renforcement.
...
Notre seconde hypothèse est qu'AFU maintient des performances plus stables lors de la transition entre apprentissage _offline_ et _online_, comparativement aux algorithmes spécialisés dans l'apprentissage _offline_ comme IQL.

Pour vérifier cette hypothèse, nous suivrons ce protocole :

1. Phase d'apprentissage _offline_ :
   - utilisation d'un ensemble de données pré-collectées par une politique sous-optimale ;
   - entraînement d'AFU et d'algorithmes de comparaison (IQL, Cal-QL) sur ces données pendant un nombre fixe d'itérations ;
   - évaluation régulière des performances pour tracer les courbes d'apprentissage _offline_.

2. Phase de transition :
   - passage progressif à l'apprentissage _online_ en permettant aux algorithmes d'interagir directement avec l'environnement ;
   - suivi précis des performances pendant cette phase critique ;
   - calcul du _transition gap_ : la différence de performance avant et après la transition.

3. Phase d'apprentissage _online_ :
   - poursuite de l'entraînement en mode entièrement _online_ ;
...
Pour l'analyse statistique, nous mesurerons :
   - l'ampleur de la chute de performance lors de la transition (_transition gap_) ;
   - le temps nécessaire pour retrouver les performances pré-transition ;
   - la performance finale après une période d'apprentissage _online_.
...
Nous utiliserons une gamme diversifiée d'environnements pour tester nos hypothèses, chacun présentant des défis spécifiques et pertinents pour évaluer les capacités d'AFU :

==== Cartpole (version continue)
Un poteau attaché à un chariot mobile, utilisé comme banc de test initial pour nos algorithmes, offrant un problème relativement simple pour valider les implémentations de base.

==== Pendulum
Pendule inversé à maintenir à la verticale, choisi spécifiquement pour sa forte variance dans les performances d'apprentissage.

==== Lunar Lander (version continue)
Simulateur d'alunissage avec espace d'action bidimensionnel, présentant le défi des gradients trompeurs qui peuvent égarer les algorithmes traditionnels.

==== Swimmer
Un environnement MuJoCo modélisant un nageur articulé devant se déplacer efficacement. Cet environnement présente des gradients trompeurs encore plus prononcés que Lunar Lander.

==== Ant Maze
Simule une fourmi robotique navigant dans différents types de labyrinthes (U-maze, Big-maze, etc.). Le défi principal de cet environnement est sa haute dimensionnalité, mais nous l'utilisons principalement pour la disponibilité de datasets pré-entraînés, ce qui le rend idéal pour évaluer la transition entre apprentissage _offline_ et _online_.
...
==== Transition gap
Pour la seconde hypothèse, nous calculerons l'AUC à partir du point de transition entre apprentissage _offline_ et _online_, fournissant une métrique robuste de l'adaptation post-transition.
...
- IQL comme représentant des algorithmes spécialisés dans l'apprentissage _offline_

Analyse et visualisation : nous produirons des whisker plots et courbes d'apprentissage permettant de comparer efficacement les performances et la stabilité des différents algorithmes, avec une attention particulière à la transition _offline_/_online_.
...
- réalisation des expériences sur la transition _offline_/_online_ ;
...
2. Quantifie précisément la stabilité d'AFU pendant la transition _offline_/_online_, en comparaison avec d'autres algorithmes spécialisés, fournissant ainsi une évaluation objective de cet aspect.
