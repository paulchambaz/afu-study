...
We conducted experiments to evaluate the performance of AFU in an off-policy settings. Our results are showned in Appendix D.

==== Cartpole
#figure(
  image("figures/Cartpole-OffPolicy-Training-Progress.svg"),
  caption: [Performance of DDPG, SAC, and AFU in the Cartpole environment under off-policy training conditions. All algorithms converge to a similar performance level, but AFU looses performance after 50k steps before converging again.],
) <cartpole-offpolicy-training-progress>
@cartpole-offpolicy-training-progress shows the training progress of DDPG, SAC and AFU in the Cartpole environment. The histogram of the last rewards of DDPG, SAC and AFU in the Cartpole environment are shown in @cartpole-ddpg-offpolicy, @cartpole-sac-offpolicy and @cartpole-afu-offpolicy respectively.
As expected, all three algorithms (DDPG, SAC, AFU) converge in this low-dimensional, low-variance problem @cartpole-offpolicy-training-progress. However, an interesting phenomenon occurs around 50k steps, where both AFU and, to a lesser extent SAC, exhibit a sudden drop in performance. One possible explanation is that after converging to a nearly optimal policy, both methods begin incorporating spurious transitions from the replay buffer generated by the random behavior policy. These out-of-distribution samples could destabilize the learned Q-values, disrupting the optimal policy.

==== MountainCar
#figure(
  image("figures/MountainCar-OffPolicy-Training-Progress.svg"),
  caption: [Performance of DDPG, SAC, and AFU in the MountainCar environment under off-policy training conditions. All algorithms converge to a similar performance level, DDPG is a little slower than SAC and AFU.],
) <mountaincar-offpolicy-training-progress>
All algorithms eventually learn to solve the task, but the reward distributions diverge widely @mountaincar-offpolicy-training-progress. AFU exhibits a clear bimodal distribution @mountaincar-afu-offpolicy, it means that it follows two different policies. SAC shows behavior consistent with a normal-like distribution centered around sub-optimal returns @mountaincar-sac-offpolicy. DDPG exhibits extreme variance @mountaincar-ddpg-offpolicy, indicating a "hit-or-miss" policy: it either successfully reaches the goal or fails completely, likely due to deterministic policy collapses conditioned by specific initial states.

==== Pendulum
#figure(
  image("figures/Pendulum-OffPolicy-Training-Progress.svg"),
  caption: [Performance of DDPG, SAC, and AFU in the Pendulum environment under off-policy training conditions. The three algorithms converge to a similar suboptimal performance level.],
) <pendulum-offpolicy-training-progress>
All algorithms converge toward moderately sub-optimal policies @pendulum-offpolicy-training-progress, but with a distinctive trimodal distribution of returns shared across algorithms @pendulum-ddpg-offpolicy, @pendulum-sac-offpolicy, @pendulum-afu-offpolicy. One plausible interpretation is that this trimodality reflects inherent metastable policies in the Pendulum environment: (1) complete failure to stabilize, (2) marginal stabilization with persistent oscillations, and (3) near-optimal stabilization with almost no oscillations. Random policy sampling appears insufficient to facilitate transitions from one mode of behavior to another, trapping learners in local basins of attraction.

==== LunarLander Continuous
#figure(
  image("figures/LunarLander-OffPolicy-Training-Progress.svg"),
  caption: [Performance of DDPG, SAC, and AFU in the LunarLander environment under off-policy training conditions. None of the algorithms converge to a stable performance level.],
) <lunarlander-offpolicy-training-progress>
None of the considered algorithms exhibit true convergence under random off-policy data @lunarlander-offpolicy-training-progress. Reward distributions remain highly skewed, with a mean trending toward zero but with extreme variance (most rewards are between -1000 and 0) @lunarlander-ddpg-offpolicy, @lunarlander-sac-offpolicy, @lunarlander-afu-offpolicy. The agents appear to learn to avoid catastrophic crashes, yet none manage to acquire the precise control strategy required for a successful landing. This failure can be attributed to the extreme sparsity of successful landing experiences in a random dataset—a classic sparse reward challenge.
...
The following figures show the performance of DDPG, SAC and AFU in an off-policy learning setting. The results are averaged over 5 runs with different random seeds. The shaded area represents the standard deviation.

#figure(
  image("figures/histogram_Cartpole_DDPG_OffPolicy.svg"),
  caption: [DDPG last rewards in the Cartpole environment under off-policy training conditions.],
) <cartpole-ddpg-offpolicy>

#figure(
  image("figures/histogram_Cartpole_SAC_OffPolicy.svg"),
  caption: [SAC last rewards in the Cartpole environment under off-policy training conditions.],
) <cartpole-sac-offpolicy>

#figure(
  image("figures/histogram_Cartpole_AFU_OffPolicy.svg"),
  caption: [AFU last rewards in the Cartpole environment under off-policy training conditions.],
) <cartpole-afu-offpolicy>

#figure(
  image("figures/histogram_MountainCar_DDPG_OffPolicy.svg"),
  caption: [DDPG last rewards in the MountainCar environment under off-policy training conditions. There is a very good reward as well as a very bad one, indicating that DDPG is either able to reach the goal or not at all.],
) <mountaincar-ddpg-offpolicy>

#figure(
  image("figures/histogram_MountainCar_SAC_OffPolicy.svg"),
  caption: [SAC last rewards in the MountainCar environment under off-policy training conditions. The distribution is normal-like around 94, indicating that SAC does not reach the optimal goal but is able to get a good reward.],
) <mountaincar-sac-offpolicy>

#figure(
  image("figures/histogram_MountainCar_AFU_OffPolicy.svg"),
  caption: [AFU last rewards in the MountainCar environment under off-policy training conditions. The distribution is bimodal, indicating that AFU is following two policies.],
) <mountaincar-afu-offpolicy>

#figure(
  image("figures/histogram_Pendulum_DDPG_OffPolicy.svg"),
  caption: [DDPG last rewards in the Pendulum environment under off-policy training conditions. The distribution is trimodal, indicating that DDPG is following three policies: a very bad, a medium and a very good one.],
) <pendulum-ddpg-offpolicy>

#figure(
  image("figures/histogram_Pendulum_SAC_OffPolicy.svg"),
  caption: [SAC last rewards in the Pendulum environment under off-policy training conditions. The distribution is trimodal, indicating that SAC is following three policies: a very bad, a medium and a very good one.],
) <pendulum-sac-offpolicy>

#figure(
  image("figures/histogram_Pendulum_AFU_OffPolicy.svg"),
  caption: [AFU last rewards in the Pendulum environment under off-policy training conditions. The distribution is trimodal, indicating that AFU is following three policies: a very bad, a medium and a very good one.],
) <pendulum-afu-offpolicy>

#figure(
  image("figures/histogram_LunarLander_DDPG_OffPolicy.svg"),
  caption: [DDPG last rewards in the LunarLander environment under off-policy training conditions. The distribution approaches values between -1000 and 0, suggesting that the algorithm doesn't learn a catastrophic policy but isn't able to land the rocket either.],
) <lunarlander-ddpg-offpolicy>

#figure(
  image("figures/histogram_LunarLander_SAC_OffPolicy.svg"),
  caption: [SAC last rewards in the LunarLander environment under off-policy training conditions. The distribution approaches values between -1000 and 0, suggesting that the algorithm doesn't learn a catastrophic policy but isn't able to land the rocket either.],
) <lunarlander-sac-offpolicy>
